...
select id, trip_id, title, occurs_at
from activities
where trip_id = $1
`

func (q *Queries) GetActivities(ctx context.Context, tripID uuid.UUID) ([]Activity, error) {
...
func (q *Queries) GetLinks(ctx context.Context, tripID uuid.UUID) ([]Link, error) {
	rows, err := q.db.Query(ctx, getLinks, tripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
